...
class User:
    ...
    def new_order(self, shop: Shop, books):
        with self.sql.transaction():
            ...
            for book in books:
                (book_id, count) = book
                ret = self.sql.execute("SELECT quantity FROM books WHERE shop_id = %s book_id = %s",
    ...
    def payment(self, order: Order):
        self.fetch()
        with self.sql.transaction():
            if self.sql.execute("SELECT * FROM orders WHERE uid = %s AND order_id = %s",
                                [self.user_id, order.order_id]) is None:
